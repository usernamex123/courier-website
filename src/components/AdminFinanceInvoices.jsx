...
export default function AdminFinanceInvoices() {
  ...
  return (
    <div className="space-y-6 font-sans w-full max-w-full overflow-hidden px-2 sm:px-4">
      {/* Header */}
          ...
          <>
            ...
            <div className="lg:hidden space-y-3 p-4">
              {filteredRecords.map((inv) => {
                ...
                return (
                  <div key={inv.id} className={`bg-gray-50 border border-gray-200 rounded-xl p-3.5 space-y-2.5 ${isChecked ? 'border-amber-400 bg-amber-50/30' : ''}`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        ...
                        />
                        <span className="font-bold text-xs text-gray-900 font-mono truncate max-w-[150px]">{invNum}</span>
                      </div>
                      <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold capitalize border border-black/5 ${PAYMENT_STATUS_STYLES[statusKey] || "bg-slate-100 text-slate-600"}`}>
                        {statusKey}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs text-gray-600">
                      <span className="text-gray-400 font-medium">Transaction Ref</span>
                      <span className="font-mono text-gray-800 truncate max-w-[180px]" title={refNum}>{refNum}</span>
                    </div>
                    <div className="flex justify-between text-xs text-gray-600">
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-gray-200/60">
                      <span className="font-extrabold text-xs text-gray-900">{fmtMoney(amount, inv.currency)}</span>
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          ...
                          className="text-yellow-700 hover:text-yellow-800 text-[11px] font-bold px-3 py-1 bg-yellow-50 rounded-lg hover:bg-yellow-100 transition-colors cursor-pointer"
                        >
                          View
                        </button>
                      </div>
